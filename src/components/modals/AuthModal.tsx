...
// ── Validasi harus Gmail asli ──
const isGmail = (email: string) => {
  const trimmed = email.trim().toLowerCase();
  // Harus diakhiri @gmail.com dan tidak ada karakter aneh
...
function getPasswordStrength(pw: string): { score: number; label: string; color: string } {
  ...
  if (score === 4) return { score, label: 'Kuat', color: 'bg-cyan-500' };
  return { score, label: 'Sangat Kuat', color: 'bg-emerald-500' };
}

// ── Rate limiter login (max 5x gagal, cooldown 15 menit) ──
...
export function AuthModal({ onClose, onSuccess, initialMode = 'login' }: AuthModalProps) {
  // Kalau initialMode reset-password, langsung ke step newpass
  const [mode, setMode] = useState<AuthMode>(
    initialMode === 'reset-password' ? 'forgot' : initialMode
  );
  ...
  // ── State khusus forgot password (2-step) ──
  ...
  const inputClass =
    'w-full px-4 py-3 rounded-2xl bg-white/5 border border-cyan-500/20 text-white placeholder-white/30 focus:outline-none focus:border-cyan-400/60 focus:bg-white/10 transition-all text-sm';
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.includes('Invalid login credentials')) {
        ...
        toast.error('Email belum dikonfirmasi. Cek inbox Gmail kamu!');
      } else {
  ...
  // ── FORGOT PASSWORD ──
  // Step 1: kirim link reset ke Gmail
  const handleForgotStep1 = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Tampilkan layar "cek email" — tunggu user klik link
      setForgotStep('otp');
      setResendCooldown(60);
      toast.success(`✅ Link reset dikirim ke ${forgotEmail}!`);
    }
  };

  // Step 2: tidak dipakai (link langsung buka modal dengan session)
  const handleForgotStep2 = async (e: React.FormEvent) => {
    e.preventDefault();
  };

  // Step 3: set password baru (sudah ada session dari link/OTP)
  const handleForgotStep3 = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.toLowerCase().includes('session')) {
        toast.error('Sesi habis. Silakan request reset password lagi.');
        switchMode('forgot');
      } else {
        toast.error(error.message);
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    // Jika sukses, Supabase akan redirect ke Google — tidak perlu close modal
  };
  ...
  // Handle klik tombol X atau backdrop
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-md bg-gradient-to-br from-[#000d14] to-[#001a24] border border-cyan-500/20 rounded-3xl shadow-[0_0_80px_rgba(6,182,212,0.25)]"
        >
          {/* Decorative glow */}
          <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full bg-cyan-500/10 blur-[80px] pointer-events-none" />
          <div className="absolute -bottom-20 -left-20 w-60 h-60 rounded-full bg-teal-500/10 blur-[80px] pointer-events-none" />
            ...
            {showCloseConfirm && (
              ...
              >
                <motion.div
                  ...
                  className="bg-gradient-to-br from-[#001a24] to-[#000d14] border border-orange-500/30 rounded-2xl p-6 w-full max-w-sm text-center shadow-[0_0_40px_rgba(249,115,22,0.2)]"
                >
          ...
          {/* Close button — z-50 tapi di bawah dialog z-[100] */}
          <button
                ...
                >
                  {mode === 'login' && (
                    <>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-cyan-400 text-xs font-semibold uppercase tracking-wider">Secure Login</span>
                      </div>
                  ...
                  {mode === 'register' && (
                    <>
                      <div className="flex items-center gap-2 mb-2">
                        <ShieldCheck className="w-5 h-5 text-cyan-400" />
                        <span className="text-cyan-400 text-xs font-semibold uppercase tracking-wider">Daftar Akun</span>
                      </div>
              ...
              {mode === 'login' && (
                ...
                >
                  <div>
                    ...
                    {email && !isGmail(email) && (
                      <p className="text-red-400 text-xs mt-1">Harus menggunakan Gmail (@gmail.com)</p>
                    )}
                  </div>
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="text-sm font-semibold text-white/70 hover:text-cyan-400 transition-colors"
                    >
                      Forgot password
                    </button>
                  </div>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.99 }}
                    className="w-full py-3.5 rounded-2xl bg-[#0f1923] hover:bg-[#162330] border border-cyan-500/20 text-white font-semibold transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: 1.01 }}
                    whileTap={{ scale: 0.99 }}
                    className="w-full py-3.5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/15 text-white font-medium transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                  >
                    {googleLoading ? (
              ...
              {mode === 'register' && (
                ...
                >
                  ...
                  {registerSuccess ? (
                    <div className="flex flex-col items-center text-center gap-4 py-4">
                      ...
                      <div>
                        ...
                        <p className="text-cyan-400 font-semibold text-sm mt-1">{email}</p>
                        <p className="text-white/40 text-xs mt-3">Klik link di email tersebut untuk mengaktifkan akun, lalu login.</p>
                      </div>
                      <motion.button
                        whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                        onClick={() => switchMode('login')}
                        className="w-full py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-semibold mt-2"
                      >
                        Ke Halaman Login
                      </motion.button>
                    </div>
                    ...
                    {password && (
                      <div className="mt-2 space-y-1">
                        <div className="flex gap-1">
                          {[1,2,3,4,5].map(i => (
                            <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= pwStrength.score ? pwStrength.color : 'bg-white/10'}`} />
                          ))}
                        </div>
                        <p className={`text-xs ${pwStrength.score >= 3 ? 'text-emerald-400' : 'text-orange-400'}`}>
                  ...
                  <motion.button type="submit" disabled={loading}
                    ...
                    className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-semibold shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                  ...
                  <motion.button type="button" onClick={handleGoogleLogin} disabled={googleLoading}
                    whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                    className="w-full py-3.5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/15 text-white font-medium transition-all flex items-center justify-center gap-3 disabled:opacity-50">
              ...
              {mode === 'forgot' && (
                ...
                >
                  <AnimatePresence mode="wait">
                    ...
                    {forgotStep === 'email' && (
                      ...
                      >
                        <div>
                          ...
                          {forgotEmail && !isGmail(forgotEmail) && (
                            <p className="text-red-400 text-xs mt-1">Harus menggunakan Gmail (@gmail.com)</p>
                          )}
                        </div>

                        <motion.button type="submit" disabled={loading || resendCooldown > 0}
                          whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                          className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-semibold shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4" />}
                    ...
                    {forgotStep === 'otp' && (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-cyan-400 font-semibold text-sm mt-1">{forgotEmail}</p>
                        ...
                        <motion.button
                          ...
                          whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                          className="w-full py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 text-sm transition-all disabled:opacity-40"
                        >
                    ...
                    {/* STEP 3 — Buat Password Baru (hanya muncul setelah klik link di email) */}
                    {forgotStep === 'newpass' && (
                      <motion.form
                        key="forgot-step-newpass"
                        initial={{ opacity: 0, x: 20 }}
                        animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: -20 }}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
                          ...
                          <span className="text-cyan-300 text-xs truncate">{forgotEmail}</span>
                        </div>
                        ...
                        <div>
                          ...
                          {newPassword && (
                            <div className="mt-2 space-y-1">
                              <div className="flex gap-1">
                                {[1,2,3,4,5].map(i => (
                                  <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= newPwStrength.score ? newPwStrength.color : 'bg-white/10'}`} />
                                ))}
                              </div>
                              <p className={`text-xs ${newPwStrength.score >= 3 ? 'text-emerald-400' : 'text-orange-400'}`}>
